...
from inverse.motionblur.motionblur import Kernel
...
def random_sq_bbox(img, mask_shape, image_size=256, margin=(16, 16)):
    ...
    box_location = "middle2" # ['random', 'middle1', 'middle2']

    if box_location == "middle1":
        # bb
        t = int(margin_height + (maxt - margin_height) / 2)
        l = int(margin_width + (maxl - margin_width) / 2)
    elif box_location == "middle2":
        t = int(margin_height + (1.2*(maxt - margin_height)) / 2)
        l = int(margin_width + (1.2*(maxl - margin_width)) / 2)
    elif box_location == "random":
        t = np.random.randint(margin_height, maxt)
        l = np.random.randint(margin_width, maxl)
    else:
        raise ValueError("Invalid box location")
...
class mask_generator:
    ...
    def _retrieve_box(self, img):
        # raise ValueError("Look it up")
        l, h = self.mask_len_range
        l, h = int(l), int(h)
        # print(f"l: {l}, h: {h}")
        mask_h = np.random.randint(l, h)
        mask_w = np.random.randint(l, h)
        # print(f"mask_h; {mask_h}, mask_w: {mask_w}")
        # raise ValueError("")
